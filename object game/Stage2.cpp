...
void Stage2::getBrick()
{
	bricks.clear();
	string path = "datas//brick2.txt";
...
void Stage2::getWall()
{
	...
	for (int i = 0; i < numObj; i++)
	{
		int l, t, w,h,min,max,climb;
		fs >> l >> t >> w >> h >>min>>max >> climb;
...
{
#pragma region play nhạc nền map 2
	sound = Sound::GetInstance();
	if(AllowSound)
	sound->Play(background2, true);
#pragma endregion
...
void Stage2::DestroyAll()
{

}

void Stage2::Update(DWORD dt)
{
	...
	if (ninja->die == false)
	{
		if (sound->isPlaying(background2) == false && AllowSound) sound->Play(background2, true);
		//chuyển map
		if (ninja->GetPosition().x >= MAP_WIDTH_2 - 27)
		{
...
#pragma region push bricks, walls and enemies
...
void Stage2::ProcessGameOver(DWORD)
{
		sound->StopAll();
...
void Stage2::RenderScoreboard()
{
	//Cột bên trái
	wsprintfW(buffer, L"Stage: 3-2 ");
